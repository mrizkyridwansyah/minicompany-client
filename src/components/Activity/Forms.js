...
export default function Forms() {
  ...
  function handleSubmit(e) {
    ...
    URI_API =
      type !== "add"
        ? `${URI_API}${id}`
        : `${URI_API}${sessionStorage.getItem("employeeId")}`;
  ...
  function handleType(e) {
    ...
    setApprovalNeeded(
      typeRef.current.value === "Business Trip" ||
        typeRef.current.value === "Overtime"
    );
  }
  ...
  function handleUpload(e) {
    ...
    fileReader.onload = () => {
      setFiles([
        ...files,
        { type: selectedFile.type, id: id, data: fileReader.result },
      ]);
    };
  ...
  function getCurrentActivity() {
    ...
    fetchQuery(
      ...
      if (data.status !== 200) {
        ...
        setApprovalNeeded(
          data.body.data.employeeActivity.type === "Business Trip" ||
            data.body.data.employeeActivity.type === "Overtime"
        );
        setError("");
      }
    });
    ...
    <>
      ...
      <Form className="mt-4" onSubmit={handleSubmit}>
        <Col md="8">
          ...
          {type !== "add" &&
            ...
            currentActivity.reimburse.length > 0 && (
              <>
                <a
                  href={
                    currentActivity.reimburse[0].file &&
                    currentActivity.reimburse[0].file !== null
                      ? currentActivity.reimburse[0].file
                      : "#"
                  }
                  download={
                    currentActivity.reimburse[0].file &&
                    currentActivity.reimburse[0].file !== null
                      ? `Attachment Reimburse ${
                          currentActivity.notes
                        } (${currentActivity.name.replace(".", " ")})`
                      : null
                  }
                >
                  <i
                  ...
                  ></i>
                  &nbsp;{" "}
                  <span className="text-weight">Download Attachment</span>
                </a>
                <br />
                <br />
              </>
            )}
          {type === "detail" && <History activityId={id}></History>}
          {type !== "detail" && (
          ...
          {
            <Link to="/activity" className="btn btn-danger" disabled={loading}>
              Cancel
            </Link>
          }
        </Col>
